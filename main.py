...
father_attack_pack = ["Из рук мага вырывается магическая энергия, формируясь в смертоносный снаряд, который направлен к вам!", "Светящиеся руны появляются вокруг мага, когда он запускает магический снаряд в вашем направлении!", "Вы видите, как Жерард II медленно вдыхает, затем выдыхает магическую бурю, направленную к вам!", "Маг взмахивает рукой, вызывая вихрь волшебных искр, которые направляются в вас!", "Маг молчит, его руки загораются магическим огнем, и мощный магический снаряд выстреливает в вашем направлении!"]
father_magic_pack = ["Слова отца вызывают у вас чувство путаницы, ухудшая вашу способность эффективно защищаться!", "Маг медленно двигает своей рукой, и ваши мысли разбегаются, делая вас менее способными к эффективной защите!", "В момент заклинания отца, вы ощущаете, как ваше внимание рассеивается, уменьшая вашу способность эффективно защищаться!"]
...
def get_text_messages(message):
    if message.text == "/start":
        ...
        player1 = Player(hp=20, kz=13, kz_flag=0, hillC=3, attack_flag=0) # изменять для тестирования праметры игрока
        flags = {'first_room_searched':0,'torch_acq':0,'first_key_acq':0,'stupid':0,'crossroads':0,'skeleton_fight':0,'gnome_killed':0,'gnome_relationship':2,'gerard':0,'who':0,'golden_key':0,'fountain_room_checked':0,'door_opened':0}
...
def callback_worker(call):
    ...
    ### НИЖЕ КОД БОЯ
        ...
        global enemy1
        enemy1 = Npc(hp=10, kz=11, kz_flag=0, attack_flag=0) # изменять для тестирования праметры противника
        bot.edit_message_reply_markup(chat_id=call.message.chat.id,message_id=call.message.message_id,reply_markup=None)
        ...
        if player1.hp <= 0:
            bot.send_message(call.message.chat.id, text='Скелет делает решительный выпад. Вы пытаетесь блокироватьего атаку, но мешкаете, и не успеваете подставить свой кинжал под его удар. Ржавый скелета прорубает ваше левое плечо, после чего застревает там. Вы не чувствуете боли, лишь только то, как ваши силы покидают вас и вам хочется прилечь отдохнуть. Просто отдохнуть...')
            time.sleep(2)
            bot.send_message(call.message.chat.id, text='Вы погибли! Чтобы начать заново, напишите "/start".')
    ...
    #
        ...
        enemy1 = Npc(hp=20, kz=12, kz_flag=0, attack_flag=0) # изменять для тестирования праметров в финальном бою
        bot.edit_message_reply_markup(chat_id=call.message.chat.id,message_id=call.message.message_id,reply_markup=None)
        #bot.send_message(call.message.chat.id,text='Повернув влево и приближаясь к источнику странных звуков, ваше сердце начинает биться сильнее от страха и неизвестности. По мере продвижения по коридору, вы видите что-то, что быстро вызывает в вас тревожное волнение.'\
        #                ' Ваш взгляд падает на живого скелета, бездумно бродящего вдоль коридора. В его руках вы видите ржавый меч. Скелет явно заметил ваше присутствие и направил свой меч в вашем направлении.',reply_markup=None)
        time.sleep(1)
        ...
        keyboard.add(key_oven)
        time.sleep(0.7)
        bot.send_message(call.message.chat.id, text="Вы проходите через позолоченную дверь в полупустую комнату с кроватью.\n-Вот и ты. Я предполагал, что, даже с чарами амнезии, долго ты там не пробудешь и в итоге навестишь своего отца - Жерарда Опэн фон Эйайовича II.\nВы видите перед собой мужчину средних лет, которого никогда в жизни не видели. \n-Я уверен, что этот проклятый гном помог тебе выбраться. Я также уверен, что он рассказал тебе, что я - твой отец. Это все правда.\nОн взял небольшую паузу в словах.\n-Ты всегда был умным мальчиком и можешь понять, почему я сейчас не дам тебе выйти. Даже с наложенным заклинанием амнезии твой внутренний талант в какой-то момент вновь проснется.\nВы чувствуете, как напряжение в воздухе нарастает.\n-Из-за твоего острого ума я не могу и продолжать запирать тебя - ты просто будешь находить способы убежать. Таким образом, мы приходим к одному возможному исходу.\nВоздух вокруг начинает покалывать, будто наэлектризован, и вдруг в вас выпускают огненное заклинание! Вы едва ли успеваете отскочить. Кажется, с ним нет смысла говорить.", reply_markup=None)
        time.sleep(5)
        bot.send_message(call.message.chat.id, "Выберите действие!", reply_markup=keyboard)
        print("inside fight")
        ...
        time.sleep(0.7)
        if enemy1.attack_flag:
            bot.send_message(call.message.chat.id, text="Вы попадаете по Жерарду Опэн фон Эйайовичу II!")
        else:
            bot.send_message(call.message.chat.id, text="Но вы не попадаете по Жерарду Опэн фон Эйайовичу II.")
        time.sleep(0.7)

        if enemy1.hp > 0:
            if random.randint(1,3) != 3:
                player1.attack()
                bot.send_message(call.message.chat.id, text=random.choice(father_attack_pack))
                time.sleep(0.7)
                if player1.attack_flag:
                    bot.send_message(call.message.chat.id, text="Жерард Опэн фон Эйайович II попадает по вам!")
                else:
                    bot.send_message(call.message.chat.id, text="Но Жерард Опэн фон Эйайович II не попадает по вам!")
                time.sleep(0.7)
            else:
                bot.send_message(call.message.chat.id, text=random.choice(father_magic_pack))
                time.sleep(0.7)
                if random.randint(1,21)-player1.kz > 0:
                    global father_magic_loop
                    father_magic_loop = 2
                    player1.kz = 12
                    bot.send_message(call.message.chat.id, text="Вы поддаетесь его чарам.")
                else:
                    bot.send_message(call.message.chat.id, text="Но ваш разум крепче его заклятий!")
            ...
            #  некогда \"живого\" скелета, который больше не представляет никакой угрозы.")
            keyboard = types.InlineKeyboardMarkup()
            key_oven = types.InlineKeyboardButton(text='Отойти от его трупа', callback_data='ending') # кнопка для выхода
            keyboard.add(key_oven)
            bot.send_message(call.message.chat.id, text="Жерард II не смог отклониться от вашей последней атаки, и вы проткнули его своим кинжалом. Он посмотрел на свою смертельную рану, посмотрел на вас, и потерял равновесие. \nПоздравляем: вы убили своего отца!", reply_markup=keyboard)
            #строку выше заменить

        print(player1.kz)
        if father_magic_loop > 0:
            father_magic_loop -= 1
        if father_magic_loop == 0:
            player1.kz = 14
        ...
        if player1.hp <= 0:
            bot.send_message(call.message.chat.id, text='Последнее заклинание, выпущенное отцом в вас, оказалось фатальным. Вы видите, как ваш отец что-то говорит, но ничего не слышать. В глазах начинает темнеть.')
            time.sleep(2)
            bot.send_message(call.message.chat.id, text='Вы погибли! Чтобы начать заново, напишите "/start".')
        elif enemy1.hp > 0 and player1.hp > 0:
            bot.send_message(call.message.chat.id,
                             text="Ваш HP: " + str(player1.hp) + " HP\nHP Жерарда Опэн фон Эйайовича II: " + str(enemy1.hp) + " HP",
                             reply_markup=keyboard)
        ...
        bot.send_message(call.message.chat.id, text=random.choice(players_safe_attack_pack))
        time.sleep(0.7)
        bot.send_message(call.message.chat.id, text="Вы попадаете по Жерарду Опэн фон Эйайовичу II и готовы отражать его атаку!")
        time.sleep(0.7)
        ...
        if enemy1.hp > 0:
            ...
            #bot.send_message(call.message.chat.id, text="С последним решительным ударом, который вы нанесли скелету,он разлетается на кучу костей с характерным звуком. Теперь вы стоите перед развалинами некогда \"живого\" скелета, который больше не представляет никакой угрозы.")
            keyboard = types.InlineKeyboardMarkup()
            key_oven = types.InlineKeyboardButton(text='Отойти от его трупа', callback_data='ending')
            keyboard.add(key_oven)
            bot.send_message(call.message.chat.id, text="Жерард II не смог отклониться от вашей последней атаки, и вы проткнули его своим кинжалом. Он посмотрел на свою смертельную рану, посмотрел на вас, и потерял равновесие. \nПоздравляем: вы убили своего отца!",
                             reply_markup=keyboard)
        if father_magic_loop > 0:
            father_magic_loop -= 1
        if father_magic_loop == 0:
            ...
            if enemy1.hp > 0:
                if random.randint(1,3) != 3:
                    ...
                    if player1.attack_flag:
                        ...
                        bot.send_message(call.message.chat.id,
                                         text="Но Жерард Опэн фон Эйайович II не попадает по вам!")
                    time.sleep(0.7)
                else:
                    bot.send_message(call.message.chat.id, text=random.choice(father_magic_pack))
                    time.sleep(0.7)
                ...
                key_oven = types.InlineKeyboardButton(text='Отойти от его трупа', callback_data='ending')
                keyboard.add(key_oven)
                bot.send_message(call.messege.chat.id, text="Жерард II не смог отклониться от вашей последней атаки, и вы проткнули его своим кинжалом. Он посмотрел на свою смертельную рану, посмотрел на вас, и потерял равновесие. \nПоздравляем: вы убили своего отца!",
                                 reply_markup=keyboard)
            if father_magic_loop > 0:
                father_magic_loop -= 1
            if father_magic_loop == 0:
